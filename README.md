...
- ✅ **毎日の目標に「歩数」を追加**: 設定 > 毎日の設定から歩数計測ON/OFFと目標歩数（デフォルト10,000歩）を設定可能に。スパイラルにも歩数ノードを追加し、Apple Healthの歩数が目標を超えると自動完了
- ✅ **スパイラルが読み込んだまま止まる不具合を修正**: Firestore取得・HealthKitフェッチにタイムアウト＋自己回復ウォッチドッグを追加。時間帯別目標設定のキャッシュTTL・保存デバウンスも整備
- ✅ **kfitへの共有（Duolingo/勉強/日記/食事）のカテゴリ分類を修正**: 食事のキーワード判定が存在せず常に「その他」になっていた不具合を修正し、共有時に実際の食事ログとしてスパイラルの食事ノードも完了するよう接続。コメントの無い画像共有もVision OCRでカテゴリ判定するフォールバックを追加
- ✅ **Web: iOSアプリ取得ボタンを設定画面に追加**、TestFlight未公開時の壊れたApp Storeリンクを修正
- ✅ **Googleログイン時にApp Store Connect API経由でTestFlight招待を自動送信**する仕組みを追加
- ✅ **全体的なパフォーマンス改善**: Cloud Functions（`calculatePoints`/`aiProxy`）の独立読み取りを並列化、HealthKitフェッチのタイムアウト保護を全経路に拡充、Webの本番バンドルをReact.lazyで分割（1,286KB→745KB、約42%減）、フィードのサムネイル画像デコードをキャッシュ経由に変更
- ✅ **未使用ファイル・コードの削除**: kedu/kmindとの相互参照をクロスチェックした上で、孤立していたSwiftビュー3件とWeb未使用コンポーネントを削除
- ✅ **Xcode署名チームIDの不整合を修正**（`project.yml`のDEVELOPMENT_TEAMを正しいApple Developerチームに統一）
