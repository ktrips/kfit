...
- [特別付録【期間限定】: Fable 5 × 5人の思考法で作る、世界を変えるアプリ](#特別付録期間限定-fable-5--5人の思考法で作る-世界を変えるアプリ)
...
2026年7月時点の Fitingo Free / Plus 機能比較です。
...
## 特別付録【期間限定】: Fable 5 × 5人の思考法で作る、世界を変えるアプリ

> この付録は期間限定の特別収録です。2026年にAnthropicが公開した最新モデル **Claude Fable 5**（Claude 5 ファミリー。従来のOpusの上位に位置する Mythos クラス）を使い、サム・アルトマン、ジェフ・ベゾス、ピーター・ティール、イーロン・マスク、スティーブ・ジョブズ ── 5人の思考法をプロンプトとして注入し、あなたのアプリの戦略を1日で組み上げる方法を紹介します。筆者が Fitingo で実際に行ったセッションの再現手順です。

### F-1. なぜ「思考法のプロンプト化」なのか

Fable 5 クラスのAIに「いいアプリのアイデアを出して」と頼むと、平均的な答えが返ってきます。AIの性能が上がるほど、**差がつくのは指示の質＝どの視点で考えさせるか**です。

偉大な起業家の思考法は、実は「質問のパターン」に分解できます。ティールは「どこで競争しないか」を問い、ベゾスは「顧客の何が変わるか」を問い、マスクは「その前提は物理法則か慣習か」を問う。この質問パターンをプロンプトに固定してしまえば、誰でも自分のアプリに5人の視点を順番に当てられます。

重要なのは順番です。**①ティール（どこで戦うか）→ ②ベゾス（何を約束するか）→ ③マスク（前提をどう壊すか）→ ④アルトマン（どう作り続けるか）→ ⑤ジョブズ（何を削るか）**。市場を選ばずに約束はできず、約束がなければ削る基準もないからです。

### F-2. ステップ①: ティール型 ─ 独占できる小さな市場を探す

[プロンプト例]: 競争しない場所を見つけさせる
```text
あなたはピーター・ティール本人ではありませんが、競争を避け、
小さく独占できる市場を探す思考法を使ってください。
対象: （あなたのアプリ）
1. 競合が多い大きな市場を分解する
2. まだ誰も強く取っていない小さな顧客群を探す
3. その顧客が強く困っている問題を特定する
4. 自分が勝てる理由を3つ作る
5. 最初に取るニッチ市場を1つに絞る
出力: 競争が激しい市場 / 分解した小市場 / 独占しやすい顧客群 / 勝てる理由 / 最初のオファー文
```

Fitingoの場合、「フィットネスアプリ市場」という赤い海が、**「健康診断で要改善判定をもらった、Apple Watchを持つ35〜50代の三日坊主」**という、大手にはROIが合わない小市場に絞り込まれました。ポイントは、Fable 5にリポジトリを読ませた状態で実行することです。自分の資産（本書のような書籍、実装済み機能）を踏まえた「勝てる理由」が出てきます。

### F-3. ステップ②: ベゾス型 ─ 未来のプレスリリースから逆算する

[プロンプト例]: 顧客の変化から逆算させる（PR/FAQ）
```text
あなたはジェフ・ベゾス本人ではありませんが、顧客から逆算する思考法を使ってください。
...
このステップの価値は、**5番の「足りない要素の指摘」**にあります。Fitingoでは「AI機能は別途APIキー要」という仕様が「作り手都合の最たるもの」と指摘されました。顧客はAPIキーという概念を知らないからです。自分では気づけない「作り手の当たり前」が、顧客の言葉で棚卸しされます。

### F-4. ステップ③: マスク型 ─ 第一原理で10分の1にする

[プロンプト例]: 前提を壊させる
```text
あなたはイーロン・マスク本人ではありませんが、第一原理で分解する思考法を使ってください。
禁止: 精神論、抽象論、名言風の回答。
1. 現在のやり方を工程ごとに分解する
2. 各工程が本当に必要か判定する
...
4. 慣習、社内都合、思い込みを削除する
5. 10倍改善案を3つ作る
6. 最初の48時間で検証できる実験に落とす
```

Fitingoでは「成長のための変更はすべてアプリ内＝審査が必要」という思い込みが壊れました。ネイティブ必須なのはセンサーとHealthKitだけで、LP・チャレンジ参加・共有ページはWebに出せば審査ゼロ、実験速度は10倍になります。**「48時間で検証できる実験」まで落とさせる**のがこのプロンプトの肝です。戦略が今週の行動に変わります。

### F-5. ステップ④: アルトマン型 ─ 1回限りの作業を資産化する

[プロンプト例]: 仕組み化・複利化させる
```text
あなたはサム・アルトマン本人ではありませんが、AI時代のスケール、
プロダクト、ネットワーク効果を考える視点を使ってください。
目的: 1回限りの作業を資産化する。
1. いま手作業で繰り返している仕事を抽出する
2. AIで半自動化できる部分を分類する
3. 一度作ると何度も使える資産へ変換する
4. ユーザーが増えるほど価値が増える仕組みを考える
5. 小さく始めて大きく伸びる導線を作る
```

Claude Code で実行すると、このステップの出力はそのまま**実装**につながります。Fitingoでは「6ターゲットのビルド検証を毎回手作業でやっている」ことが抽出され、その日のうちに `/release-check` というスキル（再利用可能な自動化手順）として保存されました。あなたのリポジトリの `.claude/skills/` に育っていく資産こそ、この付録でいちばん持ち帰ってほしいものです。

### F-6. ステップ⑤: ジョブズ型 ─ 削って1つの体験にする

[プロンプト例]: 1メッセージに圧縮させる
```text
あなたはスティーブ・ジョブズ本人ではありませんが、複雑なものを削り、
1つの体験へ圧縮する思考法を使ってください。
禁止: 全部入り、網羅、機能羅列。
1. 今の内容を要素ごとに分解する
2. 顧客が本当に覚えるべき1文を決める
3. その1文を邪魔する要素を削る
...
ステップ①〜④でアイデアは必ず増えすぎます。最後のジョブズ型は、増えた案を**顧客が覚えるべきたった1文**に圧縮する工程です。Fitingoでは5タブ・AI解析・自動カウント・SNS機能のすべてが「今度こそ、続く。」という1文の後ろに並び直されました。機能を消すのではなく、最初の10秒から消す ── それがこのステップの答えです。

### F-7. 統合: 5つの出力を1つのドキュメントにする

最後に、5ステップの出力を1本に統合させます。

[プロンプト例]: 統合戦略ドキュメントの生成
```text
これまでの5つの分析（ティール・ベゾス・マスク・アルトマン・ジョブズ）を
1つの戦略ドキュメントに統合してください。
- 冒頭に「この1枚だけ読めばいい」圧縮サマリーを置く
- 5視点の結論を1行ずつの表にする
- 統合ロードマップを「最初の48時間 / 今週 / 今月 / 四半期」で並べる
- 最後に「迷ったときの判断基準」を5つ、各思考法から1つずつ作る
- docs/strategy_plan.md として保存する
```

でき上がるのは、コンサルに数百万円払っても得られない、**あなたのコードベースと資産を前提にした**戦略書です。筆者のセッションでは、この統合ドキュメントが数時間で完成し、翌週の開発優先順位がすべて入れ替わりました。

> **最後にひとつ。** 5人の思考法は魔法ではありません。出力の質は、あなたがAIに渡す「現実」の質で決まります。リポジトリ、売上の実数、ユーザーの声 ── 恥ずかしいものほど渡してください。世界を変えるアプリは、正確な現実認識からしか生まれません。
