...
final class DuolingoTextExtractor: NSObject, ObservableObject {
    ...
    /// 外部から言語検出（NLLanguageRecognizer）を利用するための public ラッパー。
    /// テキスト共有（画像なし・単語のみ）で言語コードを推定する際に使う。
    func detectLanguagePublic(_ text: String) -> String? {
        detectLanguage(text)
    }
...
"""
    ...
    // MARK: - LLM テキスト生成（テキスト共有専用：意味 / 発音記号）
    // 画像なしで単語・フレーズだけが共有された場合、OCRでは翻訳・発音が
    // 得られないためLLMで生成する。

    /// 共有された単語・フレーズの日本語での意味を1行で返す。
    func generateMeaningJA(phrase: String, languageName: String,
                           settings: LLMSettings) async -> String? {
        let prompt = """
あなたは語学学習アシスタントです。
次の\(languageName)の単語・フレーズの日本語での意味を、簡潔に1行で答えてください（説明文や前置きは不要、訳語のみ）。
...
"""
    ...
    /// 共有された単語・フレーズの発音記号（IPA、表せない場合はカタカナ/ローマ字）を1行で返す。
    func generatePhoneticPronunciation(phrase: String, languageName: String,
                                       settings: LLMSettings) async -> String? {
        let prompt = """
あなたは語学学習アシスタントです。
次の\(languageName)の単語・フレーズの発音記号を1行のみ返してください。
IPA表記を基本とし、IPAで表しにくい言語の場合はカタカナ発音表記にしてください（説明文は不要、発音記号のみ）。
...
"""
    ...
    private func callLLMText(prompt: String, settings: LLMSettings) async -> String? {
        ...
        guard !settings.apiKey.isEmpty else {
            // UserDefaults から直接読み取ることで @MainActor 境界を回避
            ...
            // クライアント側でも activeDays を送信してエラーを受け取る
