        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            MandalaChartView(
                ...
                onTapNode: { node in
                    ...
                    case .meal:
                        let (mealType, defaultKcal, emoji, label): (MealType, Int, String, String) = {
                            switch node.slot {
                            case .morning:   return (.breakfast, 400, "🌅", "朝食")
                            case .noon:      return (.lunch,     600, "🍱", "昼食")
                            case .afternoon: return (.snack,     200, "🍫", "スナック")
                            default:         return (.dinner,    800, "🍽️", "夕食")
                            }
                        }()
                        mealQuickLogTarget = MealQuickLogTarget(
                            mealType: mealType, defaultKcal: defaultKcal, emoji: emoji, label: label
                        )
                    default:
