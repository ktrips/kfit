...
private let helpItems: [HelpItem] = [
    ...
    HelpItem(
        ...
        content: """
        ジムも、記録アプリも、3日で終わった。それはあなたの意志が弱いからではありません。記録が面倒だったからです。

        Fitingoは「今度こそ、続く」ためのアプリです。ダイエットや運動の失敗要因は「キツさ」ではなく「記録と意志力への依存」にあります。Fitingoは3つの仕組みでこれを取り除きました。

        【1. 記録ゼロ秒】
        iPhone / Apple Watch が腕立て・スクワットを自動カウント。食事は写真を撮るだけでAIがカロリーと栄養素を記録。歩数・睡眠はApple ヘルスケアから自動取得。あなたが入力する項目は原則ゼロです。

        【2. 1日90秒から】
        最初の目標は「1日1セット（約90秒）」。完了するとスパイラルとストリークが伸び、途切れそうな日は「スクワット5回だけ」と最小の行動を提案します。ストリークには3日間の猶予があります。

        【3. 見られている力】
        TOMOフィードで仲間に共有されるのは「今日やった」だけ。体重や食事の中身は共有されません。公式の90日チャレンジに参加すれば、友達がいなくても同期の仲間ができます。

        続いた日々は、スパイラルとストリークという「実績」になって残ります。10年三日坊主だった人の、最初の90秒を、Fitingoがお手伝いします。
        """
    ),
    ...
    HelpItem(
        ...
        Q10. 動作環境は？
        A. iOS 17以降のiPhone。Apple Watchは任意（watchOS 10以降）。Android版は現在ありません。
        """
    ),
    ...
    HelpItem(
        ...
        • 食事フォトAI解析の回数アップ（APIキー不要・Freeは1日1回→Plusは1日3回）
        ...
        Plusコードをお持ちの場合は「コードで解放」からも利用できます。
        """
    ),
    ...
    HelpItem(
        ...
        title: "ストリーク（連続記録）",
        content: """
        その日の到達度が60%以上、またはXPが100以上だと連続日数が+1されます。
        • 基準に届かなかった日はカウントされませんが、streak が0に戻ることはありません
        • 翌日以降また基準を達成すれば、途切れる前の連続日数からそのまま続きます
        • 90日連続達成が最初の大きな目標です
        """
    ),
    HelpItem(
