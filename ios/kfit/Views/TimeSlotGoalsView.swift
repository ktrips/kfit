...
// MARK: - 時間帯別の目標カード（トレーニング・マインドフルネス等の設定行）
// SETUP画面へのインライン表示、および TimeSlotGoalsView（スパイラル詳細シート）の両方で使用する。

struct TimeSlotGoalsCardsView: View {
...
// MARK: - TimeSlotGoalsView（スパイラル詳細シート：カード + Mandala + ストリークアラート）
