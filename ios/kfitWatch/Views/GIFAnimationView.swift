...
struct GIFAnimationView: View {
    ...
    private func loadGIF() {
        ...
        // 初回はファイル読み込み+全フレームデコードが重いためバックグラウンドで実行
        Task.detached(priority: .userInitiated) {
            ...
            let decodedFrames = imgs
            let decodedDelays = dlys
            await MainActor.run {
                WatchGIFFrameCache.store(
                    WatchGIFFrameCache.Entry(frames: decodedFrames, delays: decodedDelays),
                    for: resourceName)
