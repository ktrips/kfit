...
struct kfit_Watch_ComplicationEntryView: View {
    ...
    var body: some View {
        ...
        case .accessoryCircular:
            TrainingCircularView(entry: entry)
        case .accessoryRectangular:
            TrainingRectangularView(entry: entry)
...
struct kfit_Watch_Complication: Widget {
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: TrainingComplicationProvider()) { entry in
            kfit_Watch_ComplicationEntryView(entry: entry)
        }
...
#Preview(as: .accessoryCircular) {
    ...
    TrainingEntry(date: .now, completed: 3, goal: 5)
}
