        ...
        <div className="max-w-md w-full text-center space-y-5">
          ...
          {!user && (
            <div className="rounded-2xl p-4 text-left"
              style={{ background: 'linear-gradient(135deg,#e8f5e9,#f3e5f5)', border: '2px solid #58CC02' }}>
              <p className="font-black text-duo-dark text-sm mb-1">💾 記録を保存しよう！</p>
              <p className="text-xs text-duo-gray font-semibold mb-3">
                Googleでサインインすると、今日の記録・XP・ストリークが保存されます。
              </p>
              <button
                onClick={onFinish}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-black text-sm text-white"
                style={{ background: 'linear-gradient(135deg,#58CC02,#46A302)', boxShadow: '0 3px 0 #358B00' }}
              >
                ...
                Googleでサインインして記録を保存する
              </button>
            </div>
          )}

          <button onClick={onFinish} className={user ? 'duo-btn-primary w-full text-lg' : 'text-duo-gray font-bold text-sm underline'}>
            {user ? 'ホームへ戻る 🏠' : 'サインインせずに終わる'}
          </button>
        </div>
      </div>
    );
